...
import { LogOut, Barcode, Clock, CheckCircle } from "lucide-react";
...
export default function Conferente() {
  ...
  const biparMutation = trpc.expedicao.bipar.useMutation();
  const listarBipagens = trpc.expedicao.listar.useQuery();
  ...
  const handleBipar = async () => {
    ...
    try {
      ...
      setCodigoBarras("");
      toast.success("Nota fiscal expedida com sucesso!");
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="bg-card border-b border-border sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-foreground">Conferente de Expedição</h1>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="bg-card border border-border rounded-3xl p-8 shadow-lg mb-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-2xl font-bold text-foreground">Bipar Nota Fiscal para Expedição</h2>
          </div>
          ...
          <div className="bg-green-500/10 border border-green-500/30 rounded-2xl p-6 mb-8">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h3 className="text-lg font-semibold text-green-400 mb-2">Expedição Realizada</h3>
                <div className="space-y-1 text-sm text-foreground">
                  ...
                  <p><strong>Status:</strong> <span className="text-green-400">Expedida</span></p>
                </div>
              </div>
            </div>
          </div>
        ...
        {listarBipagens.data && listarBipagens.data.length > 0 && (
          <div className="bg-card border border-border rounded-2xl p-6">
            <h3 className="text-xl font-bold text-foreground mb-4">Histórico de Expedições</h3>
